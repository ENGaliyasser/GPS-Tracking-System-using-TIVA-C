...
break;
		...
		case 'F':
		     ...
		     SET_BIT(SYSCTL_RCGCGPIO_R,5); // set port f
		
		while (READ_BIT(SYSCTL_PRGPIO_R,5) == 0){}; // check the delay
			
         GPIO_PORTF_LOCK_R = 0x4C4F434B;// 
         GPIO_PORTF_CR_R = 0x1F; //access 0>4 pins in port f
         GPIO_PORTF_AMSEL_R = 0; // disable analog mode select
         GPIO_PORTF_AFSEL_R = 0; // no alternative fn
			// GPIO_PORTF_PCTL_R = 0;
         GPIO_PORTF_DEN_R = 0x1F; // pins 0>4 are digital
